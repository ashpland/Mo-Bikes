...
class StationTests: XCTestCase {
    ...
    var disposeBag: DisposeBag = DisposeBag()
        ...
        override func setUp() {
            ...
            stationData2 = generateStationData(stationName)
    ...
    func testStationCoordinates() {
        ...
        XCTAssertEqual(stationData1.coordinates, coordinatesString)
    }
    ...
    func testStationAvailableBikesDriver() {
        ...
        let results = try? testStation.availableBikesDriver
            .asObservable()
            .take(2)
            .do(onNext: { _ in self.testStation.stationData = self.stationData2 },
                onCompleted: { expect.fulfill() })
            .toBlocking()
            .toArray()
        
        wait(for: [expect], timeout: 1.0)
        XCTAssertEqual(results, [self.stationData1.availableBikes, self.stationData2.availableBikes])
    }
    ...
    func testStationAvailableDocksDriver() {
        ...
        wait(for: [expect], timeout: 1.0)
        XCTAssertEqual(results, [self.stationData1.availableDocks, self.stationData2.availableDocks])
    }
